    ...
    <div class="container-scroller">
        <nav class="sidebar sidebar-offcanvas" id="sidebar">
            <div class="text-center sidebar-brand-wrapper d-flex align-items-center">
                <a class="sidebar-brand brand-logo" href="{{ route('admin_dashboard') }}"><i class="mdi mdi-cart"></i>
                    Osiris</a>
                ...
                <li class="nav-item nav-profile">
                    <a href="{{ route('admin_profile') }}" class="nav-link">
                        <div class="nav-profile-image">
                            <img src="{{ asset('admin/assets/images/upload/admins/' . Auth::guard('admin')->user()->image) }}"
                                alt="profile" />
                    ...
                    <ul class="navbar-nav">
                        ...
                        <li class="nav-item dropdown">
                            <a class="nav-link count-indicator dropdown-toggle" id="notificationDropdown"
                                ...
                                @if ($orderCount)
                                    <span class="count count-varient1">{{ $orderCount }}</span>
                                @endif
                            </a>
                            <div class="dropdown-menu navbar-dropdown  preview-list"
                                ...
                                @foreach ($orderss as $item)
                                    <a class="dropdown-item preview-item" @if ($item->notification == 'notify')  style='background-color: #0260f77c' @endif
                                        href="{{ route('order', $item->OrderID) }}">
                                        <h5>{{ $item->product_name }}</h5>
                                        <div class="preview-item-content">
                                            <p class="mb-0"> new Custmer <span class="text-small text-muted"> want
                                                    to buy {{ $item->quantity }} of {{ $item->product_name }} </span>
                                            </p>
                                        </div>
                                    </a>
                                @endforeach
                                <a href="{{ route('notify') }}">
                                    <p class="p-3 mb-0">mark as read</p>
                                </a>
                            </div>
                        </li>
                    </ul>
                ...
                <footer class="footer">
                    <div class="d-sm-flex justify-content-center justify-content-sm-between">
                        ...
                        <span class="float-none float-sm-right d-block mt-1 mt-sm-0 text-center">AD Created by <a
                                href="https://www.linkedin.com/in/mohamed-abbass-b00537234?lipi=urn%3Ali%3Apage%3Ad_flagship3_profile_view_base_contact_details%3BvTlBAjpjSeirolUHnBm06g%3D%3D"
                                target="_blank"> Mohamed Abbass </a>
                        </span>
                    </div>
                </footer>
